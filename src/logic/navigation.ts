...
export function mutateStateAfterNavigation(screen : Screen, params, state : FullState, skipHistory : boolean) {    
    ...
    if (screen === Screen.Concept && state.ui.lastOpenConcept !== params.concept) {               
        state.ui.filteredItems = getFilteredItems(
            state.data.user.preferences.concepts[params.concept].filters,
            state.ui.screenParameters.concept,
        );        
        state.ui.chartConfig = { colorBy: null, sizeBy: null, posBy1: null, posBy2: null, chartType: null};        
    }
    if (screen === Screen.Instance && state.ui.lastOpenConcept !== params.concept) {               
        state.ui.filteredItems = [];        
    }
    if (screen === Screen.Concept || screen === Screen.Instance) {
